...
dateFreqDistribution <- function(heights, dates, startDate="2020-01-01", endDate="2020-06-26", 
  ...
  if (!add) {
    plotShadedAxes(xlim=c(startDate, endDate), ylim=c(ymin, ymax), ylab=ylab, yaxs='i', xaxs='i', ...)
  }
  ...
  if (!is.null(dim(heights)) && nrow(heights) > 1) {
    ...
    n <- ncol(heights)
    if (length(dates) != (n+1)) {
      stop(paste0("Error: Need one more date than number of elements in heights to delimit all intervals.", length(dates), " ", n+1))
    }
    ...
    if (plot.ci && nrow(heights) == 3) {
      ...
      if (barplot) {
        ...
        barplot2(heights[2, start:end], xlim=c(0,end-start), lty=lty, lwd=lwd, 
                 ...
                 las=2, ylim=c(ymin,ymax), width=1, space=0, xaxs='i', ylab="", xpd=FALSE, log=log)
      } else {
    ...
    if (barplot) {
      ...
      barplot2(heights[start:end], xlim=c(0,end-start), col=col, border=border, lty=lty, lwd=lwd, 
               las=2, ylim=c(ymin,ymax), width=1, space=0, xaxs='i', ylab="", xpd=FALSE, axes = FALSE, log=log)
    } else {
...
plotLagScatter <- function(clusterSummary, metadata, stat="tmrca_median", 
      ...
      # ukseqhist  <- hist(metadata$sample_date[metadata$country == "Germany"], breaks="days", plot=FALSE)
      ukseqhist  <- hist(metadata$sample_date[metadata$instate], breaks="days", plot=FALSE)
...
#' #' Lag between tmrca and oldest sequence
#' plotLagScatterState <- function(clusterSummary, metadata, stat="tmrca_median", 
#'                            startDate=as.Date("2020-01-01"), endDate=as.Date("2020-06-26"), ymax=50, method="pearson", addLine=FALSE, cex.lab=0.8, state, ...) {
#'   
#'       plotShadedAxes(xlim=c(startDate, endDate), ylim=c(0, ymax), ylab="Detection lag (days)", side=2, decimal=TRUE, yaxs='i', ...)
#'       mtext(side=1, line=4, text="Transmission lineage TMRCA", cex=cex.lab)
#'       
...
#'       par(new=TRUE)
#' 
#'       ukseqhist  <- hist(metadata$sample_date[metadata$state == state], breaks="days", plot=FALSE)
#'       ukseqs     <- c(0, cumsum(ukseqhist$counts))
#'       ukseqdates <- lubridate::decimal_date(as.Date(ukseqhist$breaks, origin=as.Date("1970-01-01")))
#' 
#'       plot(ukseqdates, ukseqs, type='l', lwd=2, col=mPal(dark$red), ylim=c(0,30000),
...
plotLineageDurations <- function(clusterSummary, clusterSamples, orderby="mostrecent",
                                       ...
                                       plotNames=TRUE, plotDurations=TRUE, state = "Germany", pal = NA, ...) {
  ...
  colMap <- lapply(dePal, function(x) mPal(x, 0.5))
  
  # fillMap <- list(mPal(ukPal$eng, 0.5),
  #                 mPal(ukPal$sct, 0.5))
  # names(fillMap) <- c(state, notstate)
  fillMap <- lapply(dePal, function(x) mPal(x, 0.5))
  ...
  legend("right", legend=labelMap, bty='n', inset=c(-1.3,0), xpd=TRUE,
         col=labelMapFill, pch=16, text.col = "#FFFFFFFF", cex=par("cex.lab"))
  legend("right", legend=labelMap, bty='n', inset=c(-1.3,0), xpd=TRUE,
         col=labelMapFill, pch=1, cex=par("cex.lab"))
...
# # Don't show individual samples before the cutoff date
# plotLineageDurationsState <- function(clusterSummary, clusterSamples, orderby="mostrecent",
#                                        startDate="2020-01-01", endDate="2020-06-26", cutoff="2019-01-01",
#                                        plotNames=TRUE, plotDurations=TRUE, state, ...) {
#   
#   plotBarStart <- function(x1, y, x2, h, n=100, col="#00000011") {
#     
#     x <- seq(0, 5, length.out=n)
...
#     clustSamples <- clusterSamples[clusterSamples$cluster == as.character(clusterSummary$cluster[i]), ]
#     #clustSamples$adm1 <- as.character(clustSamples$adm1)        
#     clustSamples$adm1 <- as.character(clustSamples$state)
#     #clustSamples$adm1[clustSamples$country != "UK"] <- "Not UK"
#     clustSamples$adm1[clustSamples$adm1 != state] <- paste0("Not ", state)
#     
#     # Plot country by country, so last country plotted is on top and most visible
#     # for (country in names(colMap)) {        
#     #   samplesX <- clustSamples$decimal_date[clustSamples$adm1 == country]
